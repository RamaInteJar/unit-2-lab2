...
app.get('/magic/:question', (req, res) => {
  ...
  const randomMagic8Response = magic8Responses[Math.floor(Math.random() * magic8Responses.length)];

  // Send the question and the Magic 8 ball response as a response
  res.send(`<h1>Your Question: ${question}</h1><h1>Magic 8 Ball Response: ${randomMagic8Response}</h1>`);
});
